...
const getKaleVersion = () => {
  let version = null;
  const exePath = getExecutablePath('kale');
  // first see if we can get a kale exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or kale is on the path
    try {
      version = child_process
