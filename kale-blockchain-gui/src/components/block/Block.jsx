...
export default function Block() {
  ...
  const poolReward = mojo_to_kale(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_kale(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const kaleFees = blockRecord.fees
    ? mojo_to_kale(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.kaleexplorer.org/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.kaleexplorer.org/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: kaleFees ? `${kaleFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Kale blockchain
            </Trans>
          </BlockTitle>
        }
        action={
