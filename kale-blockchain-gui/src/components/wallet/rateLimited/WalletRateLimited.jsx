...
import { AlertDialog, Card, Flex } from '@kale/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const kaleper_input = ip_parsed.limit;
    ...
    const kaleper_value = Number.parseInt(Number(kaleper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (kale per interval):{' '}
                  {mojo_to_kale_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
