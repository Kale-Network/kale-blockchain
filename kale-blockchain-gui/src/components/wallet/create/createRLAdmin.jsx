...
import { AlertDialog } from '@kale/core';
import {
  Typography,
  Button,
  Box,
...
import { kale_to_mojo } from '../../../util/kale';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      kaleper_input.value === '' ||
      Number(kaleper_input.value) === 0 ||
      !Number(kaleper_input.value) ||
      isNaN(Number(kaleper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const kaleper = kale_to_mojo(kaleper_input.value);
    const kaleper_value = Number.parseInt(Number(kaleper));
    ...
    // var fee = kale_to_mojo(fee_input.value);
