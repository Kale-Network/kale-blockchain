...
function BalanceCard(props: BalanceCardProps) {
  ...
  const balance_ptotal_kale = mojo_to_colouredcoin_string(balance_ptotal);
  const balance_pending_kale = mojo_to_colouredcoin_string(balance_pending);
  const balance_change_kale = mojo_to_colouredcoin_string(balance_change);
  const acc_content =
    balancebox_1 +
    balancebox_2 +
    balancebox_ptotal +
...
function SendCard(props: SendCardProps) {
  ...
  function send() {
    ...
    if (address.includes('kale_addr') || address.includes('colour_desc')) {
      dispatch(
        openDialog(
          <AlertDialog>
