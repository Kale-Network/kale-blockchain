...
import { /* mojo_to_kale_string, */ kale_to_mojo } from '../../../util/kale';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_kale_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of kale in the blockchain at the current
...
function SendCard(props: SendCardProps) {
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      dispatch(
        openDialog(
          <AlertDialog>
            <Trans>
              Error: Cannot send kale to coloured address. Please enter a kale
              address.
            </Trans>
          </AlertDialog>,
        ),
    ...
    if (address.slice(0, 12) === 'kale_addr://') {
    ...
    const amountValue = Number.parseFloat(kale_to_mojo(amount));
    const feeValue = Number.parseFloat(kale_to_mojo(fee));
