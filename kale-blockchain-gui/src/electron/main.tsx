...
import kaleEnvironment from '../util/kaleEnvironment';
import kaleConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!kaleEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    const createWindow = () => {
      ...
      if (!kaleConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !kaleConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (kaleConfig.manageDaemonLifetime()) {
        kaleEnvironment.startKaleDaemon();
      }
    };
        ...
        submenu: [
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Kale-Network/kale-blockchain/wiki',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Kale-Network/kale-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Kale-Network/kale-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Kale-Network/kale-blockchain/issues',
              );
            },
          },
          {
