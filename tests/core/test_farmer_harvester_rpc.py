...
from kale.plotting.plot_tools import stream_plot_info_ph, stream_plot_info_pk
from kale.protocols import farmer_protocol
from kale.rpc.farmer_rpc_api import FarmerRpcApi
from kale.rpc.farmer_rpc_client import FarmerRpcClient
from kale.rpc.harvester_rpc_api import HarvesterRpcApi
from kale.rpc.harvester_rpc_client import HarvesterRpcClient
...
from kale.util.block_tools import get_plot_dir
...
class TestRpc:
    ...
    async def test1(self, simulation):
        ...
        try:
            ...
            await client.set_reward_targets(encode_puzzle_hash(new_ph, "xka"), encode_puzzle_hash(new_ph_2, "xka"))
            ...
            await client.set_reward_targets(None, encode_puzzle_hash(new_ph_3, "xka"))
            ...
            assert config["farmer"]["xka_target_address"] == encode_puzzle_hash(new_ph, "xka")
            assert config["pool"]["xka_target_address"] == encode_puzzle_hash(new_ph_3, "xka")

            new_ph_3_encoded = encode_puzzle_hash(new_ph_3, "xka")
