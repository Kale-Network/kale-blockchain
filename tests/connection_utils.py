...
async def add_dummy_connection(server: KaleServer, dummy_port: int) -> Tuple[asyncio.Queue, bytes32]:
    ...
    generate_ca_signed_cert(
        server.kale_ca_crt_path.read_bytes(), server.kale_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(
        server.kale_ca_crt_path, server.kale_ca_key_path, dummy_crt_path, dummy_key_path
    )
