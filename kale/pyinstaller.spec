...
ROOT = pathlib.Path(importlib.import_module("kale").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `kale` file name (which is the executable)
    and the `kale` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `kale` directory, since the names collide.
...
version_data = copy_metadata(get_distribution("kale-blockchain"))[0]
...
# TODO: collapse all these entry points into one `kale_exec` entrypoint that accepts the server as a parameter

entry_points = ["kale.cmds.kale"] + [f"kale.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "kale.util.bip39"])

if THIS_IS_WINDOWS:
    kale_mod = importlib.import_module("kale")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/kale/util/english.txt", "kale/util"))
datas.append((f"{ROOT}/kale/util/initial-config.yaml", "kale/util"))
datas.append((f"{ROOT}/kale/wallet/puzzles/*.hex", "kale/wallet/puzzles"))
datas.append((f"{ROOT}/kale/ssl/*", "kale/ssl"))
...
add_binary("kale", f"{ROOT}/kale/cmds/kale.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/kale/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/kale/server/start_{server}.py", COLLECT_ARGS)
