...
from kale.cmds.init_funcs import kale_init
from kale.daemon.windows_signal import kill
from kale.server.server import ssl_context_for_root, ssl_context_for_server
from kale.ssl.create_ssl import get_mozilla_ca_crt
from kale.util.kale_logging import initialize_logging
from kale.util.config import load_config
from kale.util.json_util import dict_to_json_str
from kale.util.path import mkdir
from kale.util.service_groups import validate_service
from kale.util.setproctitle import setproctitle
from kale.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Kale.")
    quit()
...
service_plotter = "kale plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "kale": "kale",
        "kale_wallet": "start_wallet",
        "kale_full_node": "start_full_node",
        "kale_harvester": "start_harvester",
        "kale_farmer": "start_farmer",
        "kale_introducer": "start_introducer",
        "kale_timelord": "start_timelord",
        "kale_timelord_launcher": "timelord_launcher",
        "kale_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("kale daemon exiting in 5 seconds")
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with KALE_ROOT: {os.environ['KALE_ROOT']}")
