...
from kale.cmds.units import units
...
from kale.util.misc import format_bytes
from kale.util.misc import format_minutes
from kale.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total kale farmed: {amounts['farmed_amount'] / units['kale']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['kale']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['kale']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'kale start wallet' and 'kale wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'kale wallet show'")
    else:
        print("Note: log into your key using 'kale wallet show' to see rewards for each key")
