from kale.protocols import full_node_protocol, introducer_protocol, wallet_protocol
...
from kale.util.api_decorators import api_request, peer_required, execute_task
...
class WalletNodeAPI:
    ...
    @peer_required
    @api_request
    async def respond_removals(self, response: wallet_protocol.RespondRemovals, peer: WSKaleConnection):
        pass
    ...
    @peer_required
    @api_request
    async def respond_additions(self, response: wallet_protocol.RespondAdditions, peer: WSKaleConnection):
        pass
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: WSKaleConnection
    ):
    ...
    @peer_required
    @api_request
    async def respond_peers(self, request: full_node_protocol.RespondPeers, peer: WSKaleConnection):
        if not self.wallet_node.has_full_node():
