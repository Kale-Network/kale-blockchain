...
from kale.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from kale.protocols import wallet_protocol
from kale.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from kale.protocols.protocol_message_types import ProtocolMessageTypes
from kale.protocols.wallet_protocol import (
...
from kale.server.node_discovery import WalletPeers
...
from kale.util.errors import Err, ValidationError
from kale.util.ints import uint32, uint128
from kale.util.keychain import Keychain
from kale.util.lru_cache import LRUCache
from kale.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from kale.wallet.settings.settings_objects import BackupInitialized
...
from kale.wallet.wallet_blockchain import ReceiveBlockResult
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'kale keys' program.")
            return None
    ...
    async def get_additions(
        self, peer: WSKaleConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSKaleConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
