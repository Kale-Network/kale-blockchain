...
from kale.server.introducer_peers import IntroducerPeers
from kale.server.outbound_message import Message, NodeType
from kale.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from kale.util.errors import Err, ProtocolError
from kale.util.ints import uint16
from kale.util.network import is_localhost, is_in_network
...
class KaleServer:
    def __init__(
        ...
        kale_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        ...
        self.kale_ca_crt_path, self.kale_ca_key_path = kale_ca_crt_key
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.kale_ca_crt_path, self.kale_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
    ...
    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSKaleConnection]:
        result = []
        for _, connection in self.all_connections.items():
