...
from kale.cmds.init_funcs import check_keys
from kale.consensus.block_rewards import calculate_base_farmer_reward
from kale.pools.pool_wallet import PoolWallet
from kale.pools.pool_wallet_info import create_pool_state, FARMING_TO_POOL, PoolWalletInfo, PoolState
from kale.protocols.protocol_message_types import ProtocolMessageTypes
from kale.server.outbound_message import NodeType, make_msg
...
from kale.util.ints import uint32, uint64
from kale.util.keychain import bytes_to_mnemonic, generate_mnemonic
from kale.util.path import path_from_root
from kale.util.ws_message import WsRpcMessage, create_payload_dict
...
from kale.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_pool_sk, master_sk_to_wallet_sk
...
from kale.wallet.util.backup_utils import download_backup, get_backup_info, upload_backup
from kale.wallet.util.trade_utils import trade_record_to_dict
from kale.wallet.util.transaction_type import TransactionType
from kale.wallet.util.wallet_types import WalletType
from kale.wallet.wallet_info import WalletInfo
from kale.wallet.wallet_node import WalletNode
...
class WalletRpcApi:
    ...
    async def _state_changed(self, *args) -> List[WsRpcMessage]:
        ...
        return [create_payload_dict("state_changed", data, "kale_wallet", "wallet_ui")]
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xka_target_address")
        pool_target = config["pool"].get("xka_target_address")
        found_farmer = False
        found_pool = False
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cc_wallet":
            ...
            if request["mode"] == "new":
                ...
                from kale.pools.pool_wallet_info import initial_pool_state_from_dict
