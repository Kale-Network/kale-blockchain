...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	echo "Create kale-blockchain-$KALE_INSTALLER_VERSION.rpm"

	# shellcheck disable=SC2046
	NODE_ROOT="$(dirname $(dirname $(which node)))"

	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/Kale-Network/kale-blockchain/issues/3846
  ...
  --arch "$REDHAT_PLATFORM" --options.version $KALE_INSTALLER_VERSION \
  --license ../LICENSE
  LAST_EXIT_CODE=$?
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
