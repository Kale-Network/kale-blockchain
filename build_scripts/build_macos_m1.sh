...
electron-packager . Kale --asar.unpack="**/daemon/**" --platform=darwin \
...
if [ "$NOTARIZE" ]; then
  electron-osx-sign Kale-darwin-arm64/Kale.app --platform=darwin \
  --hardened-runtime=true --provisioning-profile=kaleblockchain.provisionprofile \
...
mv Kale-darwin-arm64 ../build_scripts/dist/
cd ../build_scripts || exit

DMG_NAME="Kale-$KALE_INSTALLER_VERSION-arm64.dmg"
echo "Create $DMG_NAME"
mkdir final_installer
electron-installer-dmg dist/Kale-darwin-arm64/Kale.app Kale-$KALE_INSTALLER_VERSION-arm64 \
...
if [ "$NOTARIZE" ]; then
  ...
  notarize-cli --file=$DMG_NAME --bundle-id net.kale.blockchain \
...
# xcrun altool --notarize-app -f Kale-0.1.X.dmg --primary-bundle-id net.kale.blockchain -u username -p password
...
# xcrun stapler staple Kale-0.1.X.dmg
#
# Validate DMG:
# xcrun stapler validate Kale-0.1.X.dmg
